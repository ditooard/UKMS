...
namespace UKMS.view
{
    public partial class kontenPasien : Form
    {
        akun akun = new akun();
        public kontenPasien()
        {
        ...
        void cariData()
        {
            if (cariTxt.Text == "")
            {
                dataGridView1.DataSource = akun.tampilSemua();
            }
            else
            {
        ...
        private void kontenPasien_Load(object sender, EventArgs e)
        {
            tampilData();
            usernameTxt.Select();
        ...
        private void ubahBtn_Click(object sender, EventArgs e)
        {
            if (akun.cekUsername(usernameTxt.Text))
            {
                if (MessageBox.Show("Data akan diubah ?", "INFORMASI", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    akun.Semester = semesterCbox.Text;
                    akun.Alamat = alamatTxt.Text;
                    akun.ubahDataPasien(usernameTxt.Text);
